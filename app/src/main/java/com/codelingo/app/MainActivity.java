...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){ super.onCreate(b); prefs=getSharedPreferences("codelingo",MODE_PRIVATE); seedCourses(); refreshState(); updateStreak(); if(prefs.getBoolean("profile_ready",false))showHome(); else showWelcome(); }
    ...
    void base(String title,boolean back){
        ...
        TextView t=text(title,24,Typeface.BOLD,DARK);top.addView(t,new LinearLayout.LayoutParams(0,dp(52),1));TextView stats=text("❤️ "+hearts+"   🔥 "+streak+"   ⚡ "+xp,13,Typeface.BOLD,DARK);top.addView(stats);root.addView(top);
    ...
    void wrong(String hint){hearts=Math.max(0,hearts-1);prefs.edit().putInt("hearts",hearts).apply();if(hearts==0){new AlertDialog.Builder(this).setTitle("❤️ Sem vidas").setMessage("Você gastou suas 5 vidas. Faça uma revisão rápida para recuperar todas agora.").setPositiveButton("REVISAR",(d,w)->{hearts=5;prefs.edit().putInt("hearts",5).apply();showHome();}).setCancelable(false).show();}else new AlertDialog.Builder(this).setTitle("Ainda não").setMessage("-1 vida. "+hint+"\n\nVidas restantes: "+hearts).setPositiveButton("TENTAR NOVAMENTE",null).show();}
